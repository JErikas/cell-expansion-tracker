...
def run():
    ...
    dir_images_seg.mkdir(parents=True, exist_ok=True)
    ...
    for czi_path in tqdm(files, desc="Converting"):
        try:
            ...
            dim_dict = dims_shape[0] if len(dims_shape) > 0 else {}
            
            # Safely extract total frames
            t_val = dim_dict.get('T', 1)
            total_frames = t_val[1] if isinstance(t_val, tuple) else int(t_val)
            
            indices = get_frame_indices(total_frames)

            for i, frame_idx in enumerate(indices):
                kwargs = {}
                if 'T' in dim_dict: kwargs['T'] = frame_idx
                if 'C' in dim_dict: kwargs['C'] = config.CHANNEL_CELL_INDEX
                
                img_data, shp = czi.read_image(**kwargs)
                img_frame = np.squeeze(img_data)
                
                while img_frame.ndim > 2:
