...
def run():
    ...
    if not avg_csv.exists():
        print("Image averages CSV not found.")
        return

    if not cell_csv.exists():
        print("Single cell CSV not found.")
        return
    ...
    plt.title(
        "Average Cell Area Change After Electroporation",
        fontsize=16,
        pad=15
    )
    ...
    plt.title(
        "Single-Cell Area Change Distribution",
        fontsize=16,
        pad=15
    )
